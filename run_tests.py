...
for i in range(1, options.secs + 1, 1):
    generate_test_files(
        test_folder, options.temp_folder, i, padding=options.padding
    )

# scan files
log_msg(
    "Running Dejavu fingerprinter on files in %s..." % test_folder,
    log=options.log,
...
log_msg(
    "finished obtaining results from dejavu in %s" % (time.time() - tm),
    log=options.log,
    silent=options.silent
)
...
for line in range(0, djv.n_lines):
    for col in range(0, djv.n_columns):
        ...
        all_match_confidence[col][line][0] = djv.result_match_confidence[line
                                                                        ][col]
...
for sec in range(0, n_secs):
    ...
    means_dvj = [
        round(x[0] * 100 / djv.n_lines, 1) for x in all_match_counter[sec]
    ]
    ...
    fig_name = os.path.join(
        options.results_folder, "matching_perc_%s.png" % test_seconds[sec]
    )
...
for sec in range(0, n_secs):
    ...
    fig_name = os.path.join(
        options.results_folder, "matching_acc_%s.png" % test_seconds[sec]
    )
    fig.savefig(fig_name)
